...
public class URLFileObject implements FileObject {
	...
	public URLFileObject(final ClientletContext context, final URL resourceURL, final String resourcePath) {
		super();
	...
	throws IOException {
		...
		try {
			return org.lobobrowser.util.io.IORoutines.loadAsText(in, this.context.getResponse().getCharset(), 4096);
		} finally {
			in.close();
		}
	}
	...
	public InputStream openInputStream() throws IOException {
		...
		if(nw.getReadyState() != NetworkRequest.STATE_COMPLETE) {
			throw new java.io.IOException("Request not complete: " + this.resourceURL + ".");
		}
		int status = nw.getStatus();
		if(status != 0 && status != 200) {
			throw new java.io.IOException("Response status is " + status + " for " + this.resourceURL + ".");
		}
	...
	public OutputStream openOutputStream() throws IOException {
		throw new java.lang.UnsupportedOperationException("Writing output to URL not supported.");
	}
	...
	public URI toUri() {
		try {
			...
			throw new java.lang.IllegalStateException("Unexpected error converting URL to URI: " + this.resourceURL + ".", mfu);
		}
	}
