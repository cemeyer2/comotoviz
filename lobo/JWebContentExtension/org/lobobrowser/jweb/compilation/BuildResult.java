...
public class BuildResult implements java.io.Serializable {
	...
	public BuildResult(final String className, final Map<String,OutputFileInfo> outputFiles, String sourceCode, Properties directives) {
	...
	public int getEstimatedTransientSize() {
		int size = 512;
		for(Map.Entry<String,OutputFileInfo> entry : this.outputFiles.entrySet()) {
	...
	public ClassLoader buildClassLoader(ClientletContext context, java.net.URL codeLocation, ClassLoader systemLoader, PathRepository classPathRepository) {	
		ClassLoader parentLoader = new PathRepositoryClassLoader(context, systemLoader, classPathRepository);
		ClassLoader finalLoader = new BuildResultClassLoader(parentLoader, codeLocation, this);
		return finalLoader;
	}
