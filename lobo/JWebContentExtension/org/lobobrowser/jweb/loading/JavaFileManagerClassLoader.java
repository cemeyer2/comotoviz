...
public class JavaFileManagerClassLoader extends SecureClassLoader {
	...
	private static final Logger logger = Logger.getLogger(JavaFileManagerClassLoader.class.getName());
	private final JavaFileManager fileManager;
	
	public JavaFileManagerClassLoader(final ClassLoader parent, final JavaFileManager fileManager) {
		super(parent);
	...
	protected Class<?> findClass(String className) throws ClassNotFoundException {
		try {
			...
			try {
				javaFileObject = this.fileManager.getJavaFileForInput(javax.tools.StandardLocation.CLASS_PATH, className, javax.tools.JavaFileObject.Kind.CLASS);
				if(javaFileObject == null) {
					throw new java.io.FileNotFoundException();
				}
			} catch(java.io.IOException ioe) {
				javaFileObject = this.fileManager.getJavaFileForInput(javax.tools.StandardLocation.CLASS_OUTPUT, className, javax.tools.JavaFileObject.Kind.CLASS);
				if(javaFileObject == null) {
			...
			if(logger.isLoggable(Level.INFO)) {
				logger.log(Level.INFO, "findClass(): Failed to find " + className + ".", ioe);
			}
	...
	protected URL findResource(String resourceName) {
		...
		try {
			FileObject fileObject = this.fileManager.getFileForInput(StandardLocation.CLASS_PATH, packageName, relativeName);
			if(fileObject == null) {
				return null;
			}
