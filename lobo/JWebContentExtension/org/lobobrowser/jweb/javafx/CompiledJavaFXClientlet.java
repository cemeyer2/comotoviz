...
//	}
	...
	@Override
	protected void compile(JavaFileManager fileManager, DiagnosticListener<? super JavaFileObject> diagnosticListener,
...
//			throw new ClientletException("System Java compiler found. You are apparently running this application with a JDK (not a JRE). For the time being, the JDK compiler needs to be bypassed in order to enable the JavaFX compiler patches. Your Java installation directory is " + System.getProperty("java.home") + ".");
//		}
		
		com.sun.javafx.api.JavafxCompiler compiler = ToolProvider.getJavafxCompiler();
	    compiler.getTask(null, fileManager, diagnosticListener, null, compilationUnits).call();	    
	}
	...
	protected URL[] getExtraPlatformClassPath() {
		...
		return new URL[] { 
			javafx.lang.FX.class.getProtectionDomain().getCodeSource().getLocation(),
			javafx.ext.swing.SwingButton.class.getProtectionDomain().getCodeSource().getLocation(),
			javafx.scene.Group.class.getProtectionDomain().getCodeSource().getLocation(),
			com.sun.scenario.Settings.class.getProtectionDomain().getCodeSource().getLocation(),
			javafx.io.http.HttpStatus.class.getProtectionDomain().getCodeSource().getLocation()
				};
	}
}
