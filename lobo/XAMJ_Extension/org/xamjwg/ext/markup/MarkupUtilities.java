...
//	public static final int MODE_RIGHT_OR_AT = 1;
	...
	public static Renderable findRenderable(Renderable[] renderables, Point point, boolean vertical) {
		return findRenderable(renderables, point, 0, renderables.length, vertical);		
	}

	public static Renderable findRenderable(Renderable[] renderables, int x, int y, boolean vertical) {
		return findRenderable(renderables, x, y, 0, renderables.length, vertical);		
	}

	private static Renderable findRenderable(Renderable[] renderables, Point point, int firstIndex, int length, boolean vertical) {
		return findRenderable(renderables, point.x, point.y, firstIndex, length, vertical);		
	}
		
	private static Renderable findRenderable(Renderable[] renderables, int x, int y, int firstIndex, int length, boolean vertical) {
		if(length == 0) {
			return null;
		}
		if(length == 1) {
			...
			return rbounds.contains(x, y) ? r : null;
		}
		else {
			...
			try {
				...
				Renderable rleft = findRenderable(renderables, x, y, firstIndex, middleIndex - firstIndex, vertical);
				if(rleft != null) {
					return rleft;
				}
