...
public class ClientletStageDelegate extends StageDelegate {
	private static final Logger logger = Logger.getLogger(ClientletStageDelegate.class.getName());
	...
	public java.awt.Component createComponent() {
		FXClassType stageDelegateType = FXContext.getInstance().findClass(this.getClass().getName());
		FXObjectValue thisValue = FXLocal.getContext().mirrorOf(this);
		FXValue stageValue = stageDelegateType.getVariable("stage").getValue(thisValue);
		if(stageValue == null) {
			throw new IllegalStateException("Delegate has no stage!");
		}
		FXClassType stageType = FXContext.getInstance().findClass(Stage.class.getName());
		FXValue sceneValue = stageType.getVariable("scene").getValue((FXObjectValue) stageValue);
		if(sceneValue == null) {
	...
	public static java.awt.Component getSceneComponent(FXValue sceneMirror) {
		...
		return (java.awt.Component)((FXLocal.ObjectValue) jsgPanelV).asObject();		
	}
