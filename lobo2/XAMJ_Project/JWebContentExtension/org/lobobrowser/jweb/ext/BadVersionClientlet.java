...
public class BadVersionClientlet implements Clientlet {
	...
	public void process(ClientletContext context) throws ClientletException {
		...
		Border cborder1 = new CompoundBorder(new EmptyBorder(10, 10, 10, 10), etchedBorder);
		Border cborder2 = new CompoundBorder(cborder1, new EmptyBorder(10, 10, 10, 10));
		textArea.setBorder(cborder2);
		textArea.setBackground(new Color(255, 200, 180));
		textArea.setEditable(false);
		textArea.setWrapStyleWord(false);
		textArea.setFont(new Font("Arial", Font.BOLD, 16));
		textArea.setText("ERROR: A JRE supporting " + feature + " is required to process this document" +
				(this.requiredVersion != null ? ", that is, JRE " + this.requiredVersion + ".\r\n" : ".\r\n") +
			    "The current Java version is " + System.getProperty("java.version") + ".\r\n");
		JScrollPane spane = new JScrollPane(textArea);
		JPanel resultingPanel = new JPanel();
		resultingPanel.setLayout(CenterLayout.getInstance());
		resultingPanel.add(spane);
		ComponentContent content = new SimpleComponentContent(resultingPanel, "Version Error", "");
