...
import org.lobobrowser.jweb.java.*;
...
public class JWebClientletSelector implements ClientletSelector {
	...
	private void checkJavaVersionForJWeb(String missingFeature, String preferredVersion) throws InvalidVersionException {
		try {
	...
	public Clientlet select(ClientletRequest request, ClientletResponse response) {
		try {
			if(response.matches("text/x-javafx-source", new String[] { ".fx" })) {
				this.checkJavaVersionForJWeb("JavaFX 1.0+", JAVAFX_REQ_VERSION);
				return new CompiledJavaFXClientlet();
			}
			else if(response.matches("text/x-java-source", new String[] { ".java" })) {
				...
				if(mimeTypeTL == null || mimeTypeTL.startsWith("text/") || mimeTypeTL.startsWith("application/x-java")) {
			...
			return new BadVersionClientlet(ve.getMissingFeature(), ve.getPreferredVersion());
		}
	}
	...
	private static class InvalidVersionException extends Exception {
		...
		public InvalidVersionException(String missingFeature, String preferredVersion) {
